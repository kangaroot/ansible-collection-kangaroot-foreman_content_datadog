# Ansible collection: kangaroot.foreman_content_datadog

Collection for configuring Datadog repositories in a Foreman/Katello installation.

The Datadog repositories that can be configured from this collection are:

- Datadog Agent v6
- Datadog Agent v7[^1]

[^1]: Enabled by default when enabling Datadog repositories.
...
foreman_content_roles:
  ...
  - kangaroot.foreman_content_datadog.foreman_content_datadog
  - ...
...
Also ensure that the Datadog repositories are enabled and at least one of the Datadog version and/or OS related repositories is enabled as well by setting the appropriate enable variables:

```yaml
foreman_enable_datadog: true
```
